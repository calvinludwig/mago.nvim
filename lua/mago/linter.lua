...
function M.lint_fix(bufnr)
  ...
  -- Run mago lint --fix (note: --fix cannot be used with --reporting-format)
...
function M.lint_only(bufnr, rules)
  ...
  if result.stdout and result.stdout ~= '' then
    ...
    if diagnostics then
      ...
      if count > 0 then
        vim.notify(string.format('[mago.nvim] Found %d issue(s) for rules: %s', count, rules_str), vim.log.levels.INFO)
      else
